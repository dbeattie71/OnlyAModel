	...
	public static class Autowire
	{
		...
		public static EventHandler<MessageEventArgs> CreateMessageHandler(int version, IEnumerable<Assembly> assemblies, params object[] observers)
		{
			...
			var actions = factories.Keys
				.ToDictionary(o => o, o => (Action<Core.Server, MessageEventArgs>)((Core.Server server, MessageEventArgs args) =>
				{
			...
			return (server, args) =>
			{
				if (actions.TryGetValue(args.Message.Type, out Action<Core.Server, MessageEventArgs> action))
				{
					action.Invoke((Core.Server)server, args);
				}
			};
		}
		...
		private static IDictionary<Type, Action<Core.Server, MessageEventArgs, object>> FindHandlers(IEnumerable<object> observers)
		{
			var handlers = new Dictionary<Type, List<Action<Core.Server, MessageEventArgs, object>>>();
			foreach (var observer in observers)
			{
				...
				foreach (var method in methods)
				{
					...
					{
						handlers.Add(type, new List<Action<Core.Server, MessageEventArgs, object>>());
					}
					handlers[type].Add((server, args, payload) => method.Invoke(observer, new object[] { server, args, payload }));
				}
			}
			return handlers.ToDictionary(kvp => kvp.Key, kvp => (Action<Core.Server, MessageEventArgs, object>)((Core.Server server, MessageEventArgs args, object payload) =>
			{
		...
		private static void ValidateHandlerMethods(IEnumerable<MethodInfo> methods)
		{
			foreach(var method in methods)
			{
				...
				if(p.Length != 3 || p[0].ParameterType != typeof(Core.Server) || p[1].ParameterType != typeof(MessageEventArgs))
				{
					var types = string.Join(", ", p.Select(o => o.ParameterType.FullName));
